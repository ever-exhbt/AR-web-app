 ...
 */
export function isSlugSafe(id: string): boolean {
 ...
 */
export function scanTargets(targetsDir: string = path.resolve('targets')): TargetFileInfo[] {
...
export function validateTargets(
  ...
  experiencesPath: string = path.resolve('content/experiences.json')
): ValidationReport {
  const errors: string[] = [];
 ...
 */
export function computeCombinedHash(targets: TargetFileInfo[]): string {
  return crypto
    .createHash('sha256')
...
export async function compileTargets({
  ...
  const images = [];
  const targetManifestList: TargetManifestEntry[] = [];
